...
DIGICAM_CONTROL은
RusticaStudio와 Nikon 카메라 사이의
카메라 제어 엔진이다.

RusticaStudio는
Nikon 카메라를 직접 제어하지 않는다.
...
- digiCamControl과 Nikon 카메라 연결 환경 사용
- RusticaStudio의 촬영 요청 전달
- Nikon 카메라 촬영 실행 환경 제공
- Nikon 촬영 결과의 PC 전달 환경 제공
...
실제 촬영 결과는
Nikon 카메라가 촬영한 이미지 파일이다.

라이브뷰 프레임을 캡처하여
Nikon 촬영 원본으로 사용하지 않는다.
...
문서에 중복 작성하지 않는다.
'@ | Set-Content `
"C:\projects\rusticastudio\docs\camera\DIGICAM_CONTROL.md" `
-Encoding UTF8